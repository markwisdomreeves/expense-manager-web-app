...
export const resetExpenseDelete = createAction("expense/delete");
...
export const deleteExpAction = createAsyncThunk(
  "expense/delete",
  async (expenseId, { rejectWithValue, getState, dispatch }) => {
    //get user token from store
    const userToken = getState()?.users?.userAuth?.token;
    const config = {
      headers: {
...
const expenseSlices = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(resetExpenseDelete, (state, action) => {
